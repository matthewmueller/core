 ...
 * @return {Promise}
 */
exports.dependencies = co.wrap(function* (file, bundl) {
  debug('%s: parsing dependencies', file.relative());
  let dir = path.dirname(file.path);
  let exts = bundl.extensions('js');
  ...
  let resolved = yield deps.map(function (dep) {
    ...
    let options = { basedir: dir, extensions: exts };

    return new Promise(function (approve, reject) {
      resolve(dep, options, function (err, result) {
        if (err) {
          reject(err);
        } else {
          approve(result);
